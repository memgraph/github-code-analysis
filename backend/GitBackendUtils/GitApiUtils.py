...
class GitApiConstants(Enum):
    ...
    trending_repos_url_format: str = "https://api.github.com/search/repositories?q=created:>{date}&per_page=100&order=desc&sort=stars"
...
class GitApiUtils:
    ...
    def get_repo_languages(self, user_repo: str) -> Optional[List]:
        return self._handle_user_api_call(GitApiConstants.repo_languages_url_format.value.format(
            user_repo=user_repo,
        ))

    def get_trending_repos(self) -> Optional[List[Dict]]:
        return self._handle_unauthenticated_api_call(GitApiConstants.trending_repos_url_format.value.format(
